...
class myarima:
    ...
    def plot_acf_pacf(self, k):
        ...
        fig = sm.graphics.tsa.plot_acf(data_diff, lags=12, ax=ax1)
        ax2 = fig.add_subplot(212)
        fig = sm.graphics.tsa.plot_pacf(data_diff, lags=12, ax=ax2)
        plt.show()
    ...
    def get_pred(self, p, d, q, start_date, end_date):
        model = sm.tsa.ARIMA(self.df, order=(p, d, q))
        model = model.fit()

        
        return model.predict(start=start_date, end=end_date, dynamic=False).to_frame(name='Value')
    ...
    def get_automodel_pred(self, pred_length):
        ...
        return model.predict(n_periods=pred_length).to_frame(name='Value') # 注意predict是相同语法
